...
class RecipeService:
    ...
    async def get_steps(self, recipe_id: int):
        """
        Возвращает шаги для указанного рецепта.
        """
    ...
    async def get_ingredients(self, recipe_id: int):
        logger.info("Запрос ингредиентов для рецепта с ID %d", recipe_id)
        
        steps_qs = await sync_to_async(RecipeStep.objects.filter)(recipe_id=recipe_id)
        logger.info("Шаги рецепта получены")
        ...
        for step in steps:
            ...
            recipe_ingredients_qs = await sync_to_async(RecipeIngredient.objects.filter)(recipe=step.recipe)
            recipe_ingredients = await sync_to_async(list)(recipe_ingredients_qs)
